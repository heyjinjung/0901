> [ARCHIVE NOTICE 2025-09-09] 이 문서는 0909.md 표준 문서에 통합되었습니다. 최신 정보는 0909.md 참고.
...
## 2025-09-09 – 단일 GOLD Unified Purchase 엔드포인트 도입

### 변경 요약
- `/api/shop/purchase` 엔드포인트 추가: conversion(모델 포인트→GOLD 전환) / item(stub 소비) 단일화.
- `ShopService.purchase_product` 신규: 멱등(idempotency_key), limit_once, gold_before/gold_delta/gold_after 계산, UserAction 로깅.
- 실시간 전역동기화: 성공 시 `broadcast_profile_update`, `broadcast_purchase_update` await 적용 (RuntimeWarning 제거).
- 테스트 추가: `test_shop_unified_purchase.py` (conversion/idempotent/item/limit_once) + `test_shop_unified_purchase_realtime.py` (브로드캐스트 호출 검증).

### 검증
- Pytest: 해당 3개 테스트 전부 PASS (unified purchase 2 + realtime 1). 기존 스모크 영향 없음.
- OpenAPI 재수출 완료 (`python -m app.export_openapi`), `/app/current_openapi.json` 3259라인에 경로 확인.
- 멱등 재요청: gold 잔액 중복 증가 없음, limit_once 2회차 400 응답 확인.
- 브로드캐스트 monkeypatch: profile/purchase 이벤트 각 1회 호출 기록.

### 다음 단계
1. 프론트엔드 상점 API 호출을 `/api/shop/purchase` 단일 경로로 교체 및 글로벌 스토어 골드 즉시 반영.
2. 동시성 레이스(idempotency_key 동일 동시 요청) 시 UniqueConstraint 충돌 재시도/단일 응답 보장 테스트 추가.
3. 전역동기화_솔루션.md & 0909데이터베이스.md에 GOLD 통합/브로드캐스트 규칙 반영 및 ECON 시나리오(환불/환불 후 재구매) 추가.
