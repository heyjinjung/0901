...
> 본 온보딩 요약은 일부 섹션에서 구매/통계 기능을 ‘계획/진행중’ 상태로 서술하고 있으나 아래 항목은 이미 구현 완료됨.
> - Unified Purchase 엔드포인트(`/api/shop/purchase`) 표준화: 멱등키(idempotency_key) + limit_once + 브로드캐스트 await.
> - 게임 통계 확장: overall_max_win / win_rate 메트릭 + Normalizer 재작성 + None dict 기본값 패턴 적용.
> - 레벨/스트릭 공식 확정: Gold=1000+200*(n-1), XP=50+25*(n-1), Level=floor(XP/500)+1.
> - 문서 내 중복/과거 표현(‘구현 예정’) 구간은 0909.md 섹션 14~18 및 `2025-09-09_게임통계_풀스택동기화_완성.md` 기준으로 ‘구현 완료’로 간주.
> 향후 필요: 이벤트 리워드 E2E / 통계 delta 브로드캐스트 / DB 제약 정리(Alembic) 반영.
> 출처: 0909.md, 0909데이터베이스.md, 게임통계 완성 보고서.
